...
@implementation MTKButtonCurrentState
...
- (id)initWithButton:(UIButton *)button controlState:(UIControlState)controlState {
    return [self initWithButton:button];
}


- (id)initWithButton:(UIButton *)button {
    self = [super initWithButton:button controlState:UIControlStateReserved];
    if (self) {
        self.button = button;
    }
    return self;
...
// Overriding this getter does all the magic.
- (UIControlState)controlState {
    return self.button.state;
}

- (void)setControlState:(UIControlState)controlState {
    // We handle UIControlStateReserved as special case, this is just implementation detail.
    NSParameterAssert(controlState == UIControlStateReserved);
}

+ (NSSet *)keyPathsForValuesAffectingControlState {
    MTKButtonCurrentState *state;
    return [NSSet setWithObjects:@keypath(state, button.state), nil];
}
